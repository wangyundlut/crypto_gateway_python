...
class binanceGateway(baseGateway):
    ...
    def load_exchange_info(self):
        ...
        for symbol in result["symbols"]:
            info = self.exchange_info_transfer(symbol)
    ...
    def get_account(self, inst_id: str, inst_type: str, ccy: str) -> accountData:
        ...
        for balance in balances:
            if balance["asset"].lower() == ccy.lower():
                ...
                acc.cash_balance = Decimal(balance["locked"])
                acc.account_risk = Decimal(10000)
                    ...
                    while True:
                        ...
                        if res['e'] == 'outboundAccountPosition':
                            ...
                            for balance in res["B"]:
                                ...
                                acc.cash_balance = Decimal(balance["l"])
                                acc.account_risk = Decimal(10000)
    ...
    async def ws_receive(self, request: Request):
        ...
        wsinfo = wsInfoData()
        wsinfo.gateway_name = self.gateway_name
        wsinfo.account_name = self.account_name
        ...
        if request.method == "post":
            wsinfo.channel = orderChannelData.ORDER
            if response.status_code // 100 == 2:
                wsinfo.ord_id = str(data["orderId"])
                wsinfo.ord_state = orderStateData.SENDSUCCEED
            elif "code" in data:
                wsinfo.code = data["code"]
                wsinfo.msg = data["msg"]
                if wsinfo.code == -1013:
                    wsinfo.msg = orderError.MINORDERSIZE
                elif wsinfo.code == -2010:
                    wsinfo.msg = orderError.POSTONLYPRICEERROR
                wsinfo.ord_state = orderStateData.SENDFAILED

        elif request.method == "delete":
            wsinfo.channel = orderChannelData.CANCELORDER
            if response.status_code // 100 == 2:
                wsinfo.ord_id = str(data["orderId"])
                wsinfo.ord_state = orderStateData.CANCELED
            elif "code" in data:
                if wsinfo.code == -2011:
                    wsinfo.msg = cancelOrderError.NOTEXIST
                wsinfo.ord_state = orderStateData.CANCELFAILED
        else:
            pass
        if self.listener_ws:
            await self.listener_ws(wsinfo)
